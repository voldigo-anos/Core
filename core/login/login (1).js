...
async function startBot(loginWithEmail) {
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			// ——————————— DEPLOY NOTIFICATION TO ADMIN ——————————— //
			try {
				const os = require("os");
				const deployTime = new Date().toLocaleString("fr-FR", { timeZone: "Africa/Abidjan" });
				const hostname = os.hostname();
				const platform = os.platform();
				const uptime = convertTime(Date.now() - global.GoatBot.startTime);
				const totalCommands = global.GoatBot.commands ? global.GoatBot.commands.size : 0;
				const deployMsg =
					`🚀 ChristusBot déployé avec succès !\n` +
					`━━━━━━━━━━━━━━━━━━━━\n` +
					`📅 Date : ${deployTime}\n` +
					`🖥️ Host : ${hostname} (${platform})\n` +
					`🤖 Bot ID : ${global.botID}\n` +
					`📦 Commandes chargées : ${totalCommands}\n` +
					`⚡ Temps de démarrage : ${uptime}\n` +
					`━━━━━━━━━━━━━━━━━━━━\n` +
					`✅ Le bot est maintenant en ligne.`;
				for (const adminUID of adminBot) {
					try {
						await api.sendMessage(deployMsg, adminUID);
					}
					catch (e) {
						log.warn("DEPLOY", `Impossible d'envoyer à l'admin ${adminUID}: ${e.message}`);
					}
				}
			}
			catch (e) {
				log.warn("DEPLOY", "Erreur notification déploiement: " + e.message);
			}
			// ————————————————————————————————————————————————————— //
